$(document).ready(function() {
    $("[name='btnFiltrar']").click(filtrarElectronicas);
    $("[name='btnListar']").click(listarTodos);
});

function filtrarElectronicas() {
    let nombre = formularioFiltrarElectronicas.nombreProducto.value.trim();
    let estado = formularioFiltrarElectronicas.estado.value.trim();
    let minPrecio = formularioFiltrarElectronicas.minPrecio.value.trim();
    let maxPrecio = formularioFiltrarElectronicas.maxPrecio.value.trim();
    if (nombre != "" && estado != "seleccione" && minPrecio != "" && maxPrecio != "") {
        if (maxPrecio > minPrecio) {
            $("#formularios").empty();
            $.get("php/filtradoelectronicas.php", ("nombre="+nombre + "&estado="+estado + "&minPrecio="+minPrecio + "&maxPrecio="+maxPrecio), function(respuesta) {
                $("#formularios").append(creaTabla(["Nombre", "Estado", "Descripcion", "Modelo", "Tipo", "Precio", "Comprar"], respuesta));
            }, "xml");
        } else {
...
function listarTodos() {
    $("#formularios").empty();
    $.get("php/disponiblesElectronicas.php", function(respuesta) {
        $("#formularios").append(creaTabla(["Nombre", "Estado", "Descripcion", "Modelo", "Tipo", "Precio", "Comprar"], respuesta));
    }, "xml");
}
...
function comprarProducto(oEvento) {
    ...
    $.post("php/comprarElectronica.php", ("id="+id), function(respuesta) {
